...
void Node::Insert(int begin, int stance) {
    if (next[build_string[begin] - 'a'] == nullptr) {
        next[build_string[begin] - 'a'] =
            new Node(begin, (int)build_string.length() - 1, stance);
        return;
    }
    ...
    if (current->start + match_shift <= current->end) {
        Node* new_node =
            new Node(current->start, current->start + match_shift - 1, -1);
        ...
        new_node->next[build_string[begin + match_shift] - 'a'] = new Node(
            begin + match_shift, (int)build_string.length() - 1, stance);
...
vector<int> Node::Find(string pattern) {
    ...
    while (last_matched < pattern.size()) {
        ...
        for (int i = current->start;
             i <= current->end && last_matched < pattern.size(); ++i) {
        ...
        if (last_matched < pattern.size() &&
            current->start + (int)last_matched <= current->end) {
            is_contain = false;
            break;
        }
    }
...
void Node::Print(int depth) {
    if (depth != 0) {
        ...
        cout << start << " " << end << " "
             << build_string.substr(start, end - start + 1) << "\t" << position
             << "\n";
    }
    for (int i = 0; i < 27; ++i) {
        Node* current = next[i];
